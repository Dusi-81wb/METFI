...
def test_generator_independent_exact_match(engine: DeterministicReconciliationEngine) -> None:
    ...
    led_dr = CanonicalLedgerEntry(
        ...
        credit=Decimal("0.00"),
        currency="INR",
        entry_timestamp=datetime(2026, 8, 1, 10, 0, tzinfo=timezone.utc),
...
def test_generator_independent_unknown_policy_safe_routing(engine: DeterministicReconciliationEngine) -> None:
...
def test_generator_deletion_regression(engine: DeterministicReconciliationEngine) -> None:
    ...
    with patch.dict(sys.modules, {"app.services.data_generator": None, "app.domain.corruption": None}):
        for fpath in fixtures_dir.glob("*.json"):
            ...
            for sc in scenarios:
                ...
                assert target_res.classification == exp_cls, (
                    ...
                    f"Expected {exp_cls}, got {target_res.classification} [{target_res.reason_code}]"
                )
